...
::::: row
:::: left
The password endpoint allows a user to update their own password.

It uses a user's JWT access token instead of an API key.

The user's JWT access token is accessible via the core JS method [Userfront.tokens.accessToken](/docs/js.html#tokens-accesstoken)
::::
:::::

<!-- ### Reset password -->

### Set password (user logged in)

::::: row
:::: left

Allows a user to set their own password when they are logged in.

<parameters path="/v0/tenants/{tenantId}/auth/password" verb="put" />

::::
:::: right

<code-samples path="/v0/tenants/{tenantId}/auth/password" verb="put" token-type="jwt" />

<response path="/v0/tenants/{tenantId}/auth/password" verb="put"/>
...
If the user has not set a password (for example, if they logged in via SSO), they are sent an email link to set their password.

If the user does not have a password, the `password` and `existingPassword` values are ignored, and the user must follow the email link.

The email link will direct the user to your Password reset path.
